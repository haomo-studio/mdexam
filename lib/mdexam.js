;(function (window, undefined) {
   ...
   * @returns {*}
   */
  mdexam.prototype.judgeCheckFun = function (jsonQ, originQj, testQj) {
    var self = this;
    switch (jsonQ.type) {
      case 'multiple-choice':
      ...
      default:
        console.warn('Unknown question type: ', jsonQ);
        return false;
    }
  };
  ...
  mdexam.prototype.jsonCheck = function (originJson, testJson) {
    ...
    _.each(testJson.questions,function (question) {
      if(this.judgeCheckFun(questionIndex[question.question], question)){
        question.score = questionIndex[question.question].score
      }
      //获取总分
      testJson.score +=  question.score
    })
