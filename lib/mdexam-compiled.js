...
;(function (window, undefined) {
  ...
  mdexam.prototype.m2j = function (mdString) {
    ...
    // 挨个题目提取json
    mdexam.json.questions = _.chain(self.distillQs(mdString)).filter(function (mdq) {
    ...
    _.each(extraRegexes, function (reg, k) {
      ...
      mdexam.json[k] = reg.test(mdString) ? mdString.match(reg)[0].replace(/^####\s{0,}\[[^\]]+\]\s{0,}/g, '').replace(/^#\s{0,}/g, '') : null;
    });
